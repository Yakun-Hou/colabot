...
def process_result(bboxs,scores,image,id):
    ...
    return a_related,None,lost_flag
def crop_array(arr):
    new_arr = np.copy(arr)  
    new_arr[1] = arr[1] + arr[3] / 2
    new_arr[3] = arr[3] / 2
    return new_arr
def get_img(name,encode):
    #print("start")
    first_frame=rospy.wait_for_message(name,Image,timeout=1)
...
if __name__=="__main__":
    ...
    got_first_frame=0
    pos_publisher = rospy.Publisher("Target_pos",code,queue_size=1)
    while(got_first_frame==0):
        
        try:
            first_frame_rgb_cv_rgb=bgr2rgb(get_img('/img_rgb_test','bgr8'))
            first_bbox=rospy.wait_for_message('bbox_test',bbox,timeout=1)
            first_frame_d_cv=get_img('/img_dpt_test','mono16')
            
            got_first_frame=1
        except:
            print("failed to get the first frame")
    first_frame=first_frame_rgb_cv_rgb
    ref_point=[int(first_bbox.x1*0.5+first_bbox.x2*0.5),int(first_bbox.y1*0.75+first_bbox.y2*0.25)]
    #print(ref_point)
    init_depth=first_frame_d_cv[ref_point[1]][ref_point[0]]*0.001
    #print(init_depth)
    
    init_bbox=np.array([first_bbox.x1, first_bbox.y1, first_bbox.x2-first_bbox.x1, first_bbox.y2-first_bbox.y1])
    init_bbox_2=crop_array(init_bbox)
    
    info={'init_bbox':init_bbox,'engine_file':"/home/unitree/test_dog/dog_3/src/detectobs/engine/body.engine",'name':'body'}
    # info_2={'init_bbox':init_bbox_2,'engine_file':"/home/unitree/test_dog/dog_3/src/detectobs/engine/leg.engine",'name':'leg'}
    ...
    # rt_2=TensorRTTracker("/home/unitree/test_dog/dog_3/src/detectobs/engine/leg.engine")
    rt=TensorRTTracker("/home/unitree/test_dog/dog_3/src/detectobs/engine/body.engine")
    tracker.initialize(first_frame,info,rt)
    cinfo=rospy.wait_for_message("/camera/aligned_depth_to_color/camera_info",CameraInfo)
    ...
    while(1):
        ...
        bboxs=[]
        scores=[]
        image_online=(get_img('/camera/color/image_raw','bgr8'))
        #result=tracker.track(_read_image(file_paths[i+1]),info)
        result=tracker.track(image_online,info)
        t1=time.time()
        bboxs.append(result['target_bbox'])
        scores.append(result["conf_score"])
        #print(result)
        # result_2=tracker_2.track(image_online,info_2)
        ...
        if i==0:
            ...
            # print(bboxs)
            init_a=bboxs[0][2]
        a_related,_,lost_flag=process_result(bboxs,scores,image_online,i)
        this_frame_d_cv=get_img('/camera/aligned_depth_to_color/image_raw','mono16')
        ...
        print('\n')
        #print(this_depth)
        pos_publisher.publish(offset)
        i=i+1
        #print(str(i)+" "+str(result_2))
        t2=time.time()
        print("infer time: "+str(t1-t0))
        print("post time: "+str(t2-t1))
