...
class STARKProcessing(BaseProcessing):
    ...
    def __init__(self, search_area_factor, output_sz, center_jitter_factor, scale_jitter_factor,
        ...
        self.transform['ps'] = self.transform['search']
    ...
    def __call__(self, data: TensorDict):
        ...
        if self.transform['joint'] is not None:
            ...
            data['ps_images'], data['ps_anno'] = self.transform['joint'](
                image=data['ps_images'], bbox=data['ps_anno'], new_roll=False)
            data['search_images'], data['search_anno'] = self.transform['joint'](
                image=data['search_images'], bbox=data['search_anno'], new_roll=False)

        for s in ['template', 'ps', 'search']:
...
class LinkerTProcessing(STARKProcessing):
...
class LinkerT2Processing(STARKProcessing):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def __call__(self, data: TensorDict):
        ...
        for s in ['template', 'search']:
            ...
            # Add a uniform noise to the center pos
            jittered_anno = [self._get_jittered_box(a, s) for a in data[s + '_pre_anno']]
