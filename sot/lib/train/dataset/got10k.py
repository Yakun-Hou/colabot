...
class Got10k(BaseVideoDataset):
    ...
    def _read_meta(self, seq_path):
        try:
            if seq_path.startswith('s3://'):  # ceph
                value = client.get(os.path.join(seq_path, 'meta_info.ini'))
                meta_info = value.decode('utf-8').split('\n')
            else:
    ...
    def _get_sequence_list(self):
        if self.root.startswith('s3://'):  # ceph
            url = os.path.join(self.root, 'list.txt')
            raw = client.get(url).decode('utf-8')
            dir_list = list(csv.reader(StringIO(raw)))
        else:
    ...
    def _read_bb_anno(self, seq_path):
        ...
        if bb_anno_file.startswith('s3://'):  # ceph
            raw = client.get(bb_anno_file)
            gt = pandas.read_csv(BytesIO(raw), delimiter=',', header=None, dtype=np.float32, na_filter=False, low_memory=False).values
        else:
    ...
    def _read_target_visible(self, seq_path):
        ...
        if seq_path.startswith('s3://'):  # ceph
            raw = client.get(occlusion_file).decode('utf-8')
            occlusion = torch.ByteTensor([int(v[0]) for v in csv.reader(StringIO(raw))])
            raw = client.get(cover_file).decode('utf-8')
            cover = torch.ByteTensor([int(v[0]) for v in csv.reader(StringIO(raw))])
        else:
