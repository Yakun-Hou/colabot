...
class COCO_ceph(COCO):
    def __init__(self, annotation_file=None):
        ...
        if not annotation_file == None:
            ...
            tic = time.time()
            if annotation_file.startswith("s3://"):  # ceph
                raw = client.get(annotation_file).decode('utf-8')
                dataset = json.load(StringIO(raw))
            else:
...
class MSCOCOSeq(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, data_fraction=None, split="train", version="2014"):
        ...
        # Load the COCO set.
        if self.anno_path.startswith("s3://"):
            self.coco_set = COCO_ceph(self.anno_path)
        else:
