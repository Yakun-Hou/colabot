...
from lib.test.utils.ceph_utils import listdir, isdir
from lib.test.utils.load_text import load_text, load_text_numpy
...
class NFSDataset(BaseDataset):
    ...
    def _construct_sequence(self, sequence_info):
        sequence_path = sequence_info['path'][10:]
        nz = sequence_info['nz']
        ...
        # frames = ['{base_path}/{sequence_path}/{sequence_path}/30/{sequence_path}/{frame:0{nz}}.{ext}'.format(
        #     base_path=self.base_path,
        #     sequence_path=sequence_path, frame=frame_num, nz=nz, ext=ext) for frame_num in
        #     range(start_frame + init_omit, end_frame + 1)]
        # anno_path = '{base_path}/{sequence_path}/{sequence_path}/30/{sequence_path}.txt'.format(
        #     base_path=self.base_path,
        #     sequence_path=sequence_path
        # )
        frames = ['{base_path}/{sequence_path}/30/{sequence_path}/{frame:0{nz}}.{ext}'.format(
            base_path=self.base_path,
            sequence_path=sequence_path, frame=frame_num, nz=nz, ext=ext) for frame_num in
            range(start_frame + init_omit, end_frame + 1)]
        anno_path = '{base_path}/{sequence_path}/30/{sequence_path}.txt'.format(
            base_path=self.base_path,
            sequence_path=sequence_path
        )
        ...
        try:
            ground_truth_rect = load_text_numpy(anno_path, delimiter=' ', dtype=str)
