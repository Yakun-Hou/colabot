...
from lib.models.ostrack import build_linker_m
...
class LinkerM(BaseTracker):
    def __init__(self, params, dataset_name):
        super().__init__(params)
        network = build_linker_m(params.cfg, training=False)
    ...
    def track(self, image, info: dict = None):
        ...
        with torch.no_grad():
            out_dict = self.network.forward(self.template, search, self.memory, ce_template_mask=self.box_mask_z)
        ...
        if max_score >= 0.65 and pred_score >= 0.5:
        # if True:
            self.ps_patch_arr = x_patch_arr.copy()
            self.memory = out_dict['memory']
