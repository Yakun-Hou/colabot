...
from lib.models.ostrack import build_linker_rnn
...
from lib.utils.visualize import linker_rnn_vis


class LinkerRNN(BaseTracker):
    def __init__(self, params, dataset_name):
        super().__init__(params)
        network = build_linker_rnn(params.cfg, training=False)
        ...
        self.vis_layers = [3, 6, 9, 11]  # visualize the 4, 7, 10, 12 layer's attention
        self.frame_id = 0
        if self.debug:
            if not self.use_visdom:
                ...
                # self.add_hook()
                # self._init_visdom(visdom_info={'server': 'http://cluster-proxy.sh.sensetime.com', 'port': 19939}, debug=1)
    ...
    def track(self, image, info: dict = None):
                ...
                for i in self.vis_layers:  # layer_idx
                    hooks.append(
                        self.network.backbone.blocks[i].attn.register_forward_hook(
                            lambda self, input, output: attn_weights.append(output[2])
                        ))
            out_dict, debug_info = self.network.forward(
                self.template, self.ps, search, ce_template_mask=self.box_mask_z, debug=self.debug)
        ...
        # for debug
        if self.debug:# and self.frame_id > 30:
            ...
            if not self.use_visdom:
                ...
                cv2.putText(image_BGR, '%.2f' % float(max_score), (10, 60),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
                cv2.putText(image_BGR, '%.2f' % float(pred_score), (10, 90),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
                ...
                # save_path = os.path.join(self.save_dir, "%04d.jpg" % self.frame_id)
                # save_path = None
                # linker_rnn_vis(debug_info, ps=self.ps_patch_arr, save_path=save_path, attn_weights=attn_weights, vis_layers=self.vis_layers)
            else:
                cv2.putText(image, '%.2f' % float(max_score), (10, 80),
                            cv2.FONT_HERSHEY_SIMPLEX, 3, (255, 0, 0), 2, cv2.LINE_AA)
                if info['gt_bbox'] is None or math.isnan(info['gt_bbox'][0]):
                    self.visdom.register((image, self.state), 'Tracking', 1, 'Tracking')
                else:
                    self.visdom.register((image, info['gt_bbox'].tolist(), self.state), 'Tracking', 1, 'Tracking')
                self.visdom.register(torch.from_numpy(x_patch_arr).permute(2, 0, 1), 'image', 1, 'search_region')
                if self.ps_patch_arr is not None:
        ...
        # if max_score >= 0.6 and pred_score >= 0.5:
        if True:
            self.ps_patch_arr = x_patch_arr.copy()
            self.ps = copy.deepcopy(out_dict['t_feat'])
