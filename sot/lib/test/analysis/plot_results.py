...
def check_and_load_precomputed_results(trackers, dataset, dataset_name, report_name, force_evaluation=False, **kwargs):
    # Load data
    settings = env_settings()

    # Load pre-computed results
    result_plot_path = os.path.join(settings.result_plot_path, dataset_name)
    eval_data_path = os.path.join(result_plot_path, '%s.pkl' % report_name)

    if os.path.isfile(eval_data_path) and not force_evaluation and None:
        with open(eval_data_path, 'rb') as fh:
            eval_data = pickle.load(fh)
    else:
        # print('Pre-computed evaluation data not found. Computing results!')
        eval_data = extract_results(trackers, dataset, dataset_name, **kwargs)
...
def get_auc_curve(ave_success_rate_plot_overlap, valid_sequence):
    ...
    for a in ave_success_rate_plot_overlap.mean(-1):
        print(a.item())
...
def plot_results(trackers, dataset, dataset_name, report_name, merge_results=False,
    ...
    # Load pre-computed results
    result_plot_path = os.path.join(settings.result_plot_path, dataset_name)
    eval_data = check_and_load_precomputed_results(trackers, dataset, dataset_name, report_name, force_evaluation, **kwargs)
    ...
    print('\nGenerating plots for: {}'.format(dataset_name))
...
def print_results(trackers, dataset, dataset_name, report_name, merge_results=False,
    ...
    """
    # Load pre-computed results
    eval_data = check_and_load_precomputed_results(trackers, dataset, dataset_name, report_name=report_name, **kwargs)
    ...
    report_text = generate_formatted_report(tracker_disp_names, scores, table_name=dataset_name + ': ' + report_name)
    print(report_text)
...
def print_per_sequence_results(trackers, dataset, dataset_name, report_name, merge_results=False,
    ...
    """
    # Load pre-computed results
    eval_data = check_and_load_precomputed_results(trackers, dataset, dataset_name, report_name, **kwargs)
...
def print_results_per_video(trackers, dataset, dataset_name, report_name, merge_results=False,
