...
class BaseBackbone(nn.Module):
    ...
    def finetune_track(self, cfg, patch_start_index=1):
        ...
        ps_size = to_2tuple(cfg.DATA.PRE_SEARCH.SIZE)
        ...
        ps_patch_pos_embed = nn.functional.interpolate(patch_pos_embed, size=(new_P_H, new_P_W), mode='bicubic',
                                                       align_corners=False)
        ps_patch_pos_embed = ps_patch_pos_embed.flatten(2).transpose(1, 2)
        ...
        self.pos_embed_ps = nn.Parameter(ps_patch_pos_embed)
    ...
    def forward_features(self, z, ps, x):
        ...
        aux_dict = {"attn": None}
        # return self.norm(x), aux_dict, [z, x]
        return self.norm(x), aux_dict, [x[:,:64],x[:,-256:]]
        # return self.norm(x), aux_dict, [x[:,:144],x[:,-576:]]
    ...
    def forward(self, z, ps, x, **kwargs):
        ...
        Returns:
            ...
            attn : None
        """
        x, aux_dict, s = self.forward_features(z, ps, x)
