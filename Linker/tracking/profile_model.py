...
import time
import importlib
from lib.models.ostrack.linker import build_linker
...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str, default='linker', choices=['ostrack', 'linker'],
                        help='training script name')
    parser.add_argument('--config', type=str, default='256_full_score_369', help='yaml configure file name')
...
def evaluate_vit_linker(model, template, ps, search):
    '''Speed Test'''
    macs1, params1 = profile(model, inputs=(template, ps, search),
                             custom_ops=None, verbose=False)
    macs, params = clever_format([macs1, params1], "%.3f")
    print('overall macs is ', macs)
    print('overall params is ', params)
...
if __name__ == "__main__":
    ...
    if args.script == "linker":
        ...
        if merge_layer <= 0:
            evaluate_vit_linker(model, template, ps, search)
        else:
            evaluate_vit_separate(model, template, search)
    elif args.script == "ostrack":
        model_module = importlib.import_module('lib.models')
        # model_constructor = model_module.build_ostrack
        # model_constructor = model_module.build_linker
        model_constructor = build_ostrack
        model = model_constructor(cfg, training=False)
        # get the template and search
        template = torch.randn(bs, 3, z_sz, z_sz)
        # ps = torch.randn(bs, 3, x_sz, x_sz)
