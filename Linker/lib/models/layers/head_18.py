...
class FCN(nn.Module):
    def __init__(self, in_planes, out_planes, channel, freeze_bn=False, activation=None):
        super().__init__()
        self.model = nn.Sequential(*[
            conv(in_planes, channel, freeze_bn=freeze_bn),
            conv(channel, channel // 2, freeze_bn=freeze_bn),
            conv(channel // 2, channel // 4, freeze_bn=freeze_bn),
            conv(channel // 4, channel // 8, freeze_bn=freeze_bn),
            nn.Conv2d(channel // 8, out_planes, kernel_size=1)
        ])
...
class ScoreHead(nn.Module):
    def __init__(self, in_planes, channel, mlp_in, freeze_bn=False):
        super().__init__()
        self.fcn = FCN(in_planes=in_planes, out_planes=1, channel=channel, freeze_bn=freeze_bn)
        self.mlp = MLP(input_dim=mlp_in, hidden_dim=256, output_dim=1, num_layers=3, BN=False)
