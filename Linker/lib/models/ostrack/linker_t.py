...
class Attention(nn.Module):
    ...
    def forward(self, x, n_t, n_s, mask=None, return_attention=False):
        # x: B, N, C
        # mask: [B, N, ] torch.bool
...
class CEBlock(nn.Module):
    ...
    def forward(self, x, global_index_template, global_index_search, mask=None,
        ...
        x_attn, attn = self.attn(self.norm1(x), lens_t, lens_s, mask, True)
...
class VisionTransformerCE(VisionTransformer):
    ...
    def forward_features(self, z, ps, s, mask_z=None, mask_x=None,
    ...
    def forward(self, z, ps, x, ce_template_mask=None, ce_keep_rate=None,
                tnc_keep_rate=None,
                return_last_attn=False):
        return self.forward_features(z, ps, x, ce_template_mask=ce_template_mask, ce_keep_rate=ce_keep_rate)
...
class LinkerT(nn.Module):
    ...
    def forward(self,
        ...
        x, aux_dict = self.backbone(z=template, ps=ps, x=search,
...
def build_linker_t(cfg, training=True):
    ...
    if 'LinkerT' in cfg.MODEL.PRETRAIN_FILE and training:
        checkpoint = torch.load(cfg.MODEL.PRETRAIN_FILE, map_location="cpu")
        checkpoint["net"].pop('backbone.pos_embed_ps')  # using Linker_ori checkpoint
