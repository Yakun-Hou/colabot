...
class Attention(nn.Module):
    ...
    # def attn_m(self, q, k, v, k_m, v_m):
    #     attn = (q @ k.transpose(-2, -1)) * self.scale
    #     attn_m = (q @ k_m.transpose(-2, -1)) * self.scale
    #     sim = torch.cat((attn, attn_m), dim=-1)
    #     # attn = sim.softmax(dim=-1)
    #     attn = self.stable_softmax(sim)
    #     attn = self.attn_drop(attn)
    #     raise NotImplementedError
    #     attn_l, attn_m = attn[] #TODO:comfirm dim after KNN
    #     x_l = attn_l @ v
    #     x_m = attn_m @ v
    #     x = x_l + x_m
    #     x = rearrange(x, 'b h t d -> b t (h d)')
    ...
    def forward(self, x, kv, memory, n_t, n_s, mask=None, return_attention=False):
        # x: B, N, C
        # mask: [B, N, ] torch.bool
        ...
        if not memory:
            ...
            # print('not memory')
            # import ipdb; ipdb.set_trace()
            # to_kv = self.kv(x).reshape(B, N, 2, C // self.num_heads).permute(2, 0, 1, 3)
            # k, v =to_kv.unbind(0)
            # k = k.unsqueeze(1)
            # v = v.unsqueeze(1)
            x, attn = self.attn(q, k, v)
            attn_t = attn[:, :, :n_t]  # (B, h, n_t, n_t+n_s, C//h)
        else:
            # t, s, ps = torch.split(x, [n_t, n_s, N-n_t-n_s], dim=1)

            # B, N, C = t.shape
            # qkv_t = self.qkv(t).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            # q_t, k_t, v_t = qkv_t.unbind(0)

            # B, N, C = s.shape
            # qkv_s = self.qkv(s).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            # q_s, k_s, v_s = qkv_s.unbind(0)
            # # q_s，k_ps，v_ps
            # B, N, C = ps.shape
            # qkv_ps = self.qkv(ps).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            # _, k_ps, v_ps = qkv_ps.unbind(0)

            # # template attention
            # t, attn_t = self.attn(q_t, torch.cat((k_t, k_s), dim=2), torch.cat((v_t, v_s), dim=2))

            # # search attention
            # s, attn_s = self.attn(q_s, torch.cat((k_t, k_s, k_ps), dim=2), torch.cat((v_t, v_s, v_ps), dim=2))
            ...
            # import ipdb; ipdb.set_trace()
            topk = 32
            kv = memory.search(q, topk = topk)
            mem_k, mem_v = kv[0].unbind(dim = -2)
            # D_k, I_k = memory[0].search(q, 32) # actual search 
            # D_v, I_v = memory[1].search(q, 32) # actual search
            # mem_k, mem_v =  
            # import ipdb; ipdb.set_trace()
            q = rearrange(q, 'b t (h d) -> b h t d', h=self.num_heads)
            mem_k = rearrange(mem_k, 'b t n (h d) -> b h t n d', h=self.num_heads)
            mem_v = rearrange(mem_v, 'b t n (h d) -> b h t n d', h=self.num_heads)
            sim_mem = einsum('b h i d, b h i j d -> b h i j', q, mem_k)
            sim = (q @ k.transpose(-2, -1)) * self.scale
            sim = torch.cat((sim_mem, sim), dim = -1)
            attn = self.stable_softmax(sim)
            attn = self.attn_drop(attn)
            local_attn, mem_attn = attn[..., topk:], attn[..., :topk]
            local_out = einsum('b h i j, b h j d -> b h i d', local_attn, v)
            ...
            if return_attention:
                return out, attn_t, torch.stack((k, v), dim = -2).detach()
            return out, torch.stack((k, v), dim = -2).detach()
            # x, attn = self.attn(q, k, v)
            # kv[0] = rearrange(kv[0], 'b t (h d)-> b h t d', h=self.num_heads)
            # kv[1] = rearrange(kv[1], 'b t (h d)-> b h t d', h=self.num_heads)
            # x_m, attn_m = self.attn(q, kv[0], kv[1])
            # x, attn = self.attn_m(q, k, v, kv[0], kv[1])
        ...
        v = rearrange(v, 'b h t d -> b t (h d)')
        if return_attention:
            return x, attn_t, torch.stack((k, v), dim = -2).detach()
        return x, torch.stack((k, v), dim = -2).detach()
...
class CEBlock(nn.Module):
    ...
    def forward(self, x, kv, memory, global_index_template, global_index_search, mask=None,
                ce_template_mask=None, keep_ratio_search=None):
        lens_t = global_index_template.shape[1]
        lens_s = global_index_search.shape[1]
        x_attn, attn, kv_r = self.attn(self.norm1(x), kv, memory, lens_t, lens_s, mask, True)
        x = x[:, :lens_t+lens_s] + self.drop_path(x_attn)
        ...
        return x, global_index_template, global_index_search, removed_index_search, attn, kv_r
...
class BaseBackbone(nn.Module):
    ...
    def finetune_track(self, cfg, patch_start_index=1):
        ...
        self.pos_embed_ps = nn.Parameter(torch.zeros(1, 1, self.embed_dim))
        trunc_normal_(self.pos_embed_ps, std=.02)
    ...
    def forward_features(self, z, ps, x):
        ...
        lens_x = self.pos_embed_s.shape[1]
...
class VisionTransformerCE(VisionTransformer):
    ...
    def forward_features(self, z, l_kv, memory, s, mask_z=None, mask_x=None,
        ...
        if self.temporal_mode == 'rnn':
            l_s_feat = []
            for i, blk in enumerate(self.blocks):
                if ps is not None:
                    x = torch.cat((x, ps[i] + self.pos_embed_ps), dim=1)
                x, global_index_t, global_index_s, removed_index_s, attn = \
                    blk(x, global_index_t, global_index_s, mask_x, ce_template_mask, ce_keep_rate)
                if self.ce_loc is not None and i in self.ce_loc:
                    removed_indexes_s.append(removed_index_s)
                l_s_feat.append(x[:, lens_z:].clone().detach())
        elif self.temporal_mode == 'transformer-xl':
            l_s_feat = [x[:, lens_z:].clone().detach()]
            for i, blk in enumerate(self.blocks):
                x, global_index_t, global_index_s, removed_index_s, attn = \
                    blk(x, None if ps is None else ps[i] + self.pos_embed_ps, global_index_t, global_index_s, mask_x, ce_template_mask, ce_keep_rate)
                if self.ce_loc is not None and i in self.ce_loc:
                    removed_indexes_s.append(removed_index_s)
                if i < len(self.blocks) - 1:
                    l_s_feat.append(x[:, lens_z:].clone().detach())
        elif self.temporal_mode == 'memvit':
            l_s_feat = [x[:, lens_z:].clone().detach()]
            for i, blk in enumerate(self.blocks):
                # import ipdb; ipdb.set_trace()
                # x, global_index_t, global_index_s, removed_index_s, attn, kv_r = \
                #     blk(x, None if ((len(l_kv) <= 2) or i !=8) else l_kv[-1], memory if i==8 else None, global_index_t, global_index_s, mask_x, ce_template_mask, ce_keep_rate)
                x, global_index_t, global_index_s, removed_index_s, attn, kv_r = \
                    blk(x, None, memory if (i==8 and len(l_kv)>0) else None, global_index_t, global_index_s, mask_x, ce_template_mask, ce_keep_rate)
                # if len(l_kv) != 12:
                #     l_kv.append(kv_r)
                if i == 8:
                    l_kv = kv_r
            ...
            #     ce_template_mask=None, keep_ratio_search=None):
        elif self.temporal_mode == 'bevformer':
            raise NotImplementedError
        elif self.temporal_mode == 'rmt':  # Recurrent Memory Transformer
            raise NotImplementedError
        else:
            raise NotImplementedError
        ...
        if removed_indexes_s and removed_indexes_s[0] is not None:
            ...
            C = s.shape[-1]
            s = torch.zeros_like(s).scatter_(dim=1, index=index_all.unsqueeze(-1).expand(B, -1, C).to(torch.int64),
                                             src=s)
    ...
    def forward(self, z, l_kv, memory, x, ce_template_mask=None, ce_keep_rate=None,
                tnc_keep_rate=None,
                return_last_attn=False):
        return self.forward_features(z, l_kv, memory, x, ce_template_mask=ce_template_mask, ce_keep_rate=ce_keep_rate)
    ...
    if pretrained:
        if 'npz' in pretrained:
            ...
            try:
                ...
                print('Failed Loading pretrained model from: ' + pretrained)
...
class LinkerT(nn.Module):
    ...
    def forward(self,
        ...
        x, aux_dict = self.backbone(z=template, l_kv=l_kv, memory=memory, x=search,
...
def build_linker_mem(cfg, training=True):
    ...
    if cfg.MODEL.PRETRAIN_FILE and ('LinkerT' not in cfg.MODEL.PRETRAIN_FILE) and training:
    ...
    if cfg.MODEL.BACKBONE.TYPE == 'vit_base_patch16_224_ce':
        backbone = vit_base_patch16_224_ce(pretrained, drop_path_rate=cfg.TRAIN.DROP_PATH_RATE,
                                           ...
                                           temporal_mode=cfg.MODEL.TEMPORAL_MODE
                                           )
    ...
    if 'LinkerMem' in cfg.MODEL.PRETRAIN_FILE and training:
