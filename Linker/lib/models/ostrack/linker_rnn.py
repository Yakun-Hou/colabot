...
class Attention(nn.Module):
    ...
    def forward(self, x, n_t, n_s, mask=None, return_attention=False):
        ...
        if N == n_t + n_s:
            ...
            q_t, k_t, v_t = qkv_t.unbind(0)  # (B, h, N, C//h)
...
class CEBlock(nn.Module):
    ...
    def forward(self, x, global_index_template, global_index_search, mask=None,
        ...
        x_attn, attn_t, _ = self.attn(self.norm1(x), lens_t, lens_s, mask, True)
        ...
        if self.keep_ratio_search < 1 and (keep_ratio_search is None or keep_ratio_search < 1):
            ...
            x, global_index_search, removed_index_search = \
                candidate_elimination(attn_t, x, lens_t, lens_s, keep_ratio_search, global_index_search, ce_template_mask)
...
class VisionTransformerCE(VisionTransformer):
    ...
    def forward_features(self, z, ps, s, mask_z=None, mask_x=None,
        ...
        for i, blk in enumerate(self.blocks):
            ...
            l_t_feat.append(x[:, :lens_z].clone().detach())
    ...
    def forward(self, z, ps, x, ce_template_mask=None, ce_keep_rate=None,
                tnc_keep_rate=None,
                return_last_attn=False, debug=False):
        return self.forward_features(z, ps, x, ce_template_mask=ce_template_mask, ce_keep_rate=ce_keep_rate, debug=debug)
...
class LinkerRNN(nn.Module):
    ...
    def forward(self,
        ...
        x, aux_dict = self.backbone(z=template, ps=ps, x=search,
                                    ...
                                    return_last_attn=return_last_attn, debug=debug)
...
def build_linker_rnn(cfg, training=True):
    current_dir = os.path.dirname(os.path.abspath(__file__))  # This is your Project Root
    pretrained_path = os.path.join(current_dir, '../../../pretrained_models')
    if cfg.MODEL.PRETRAIN_FILE and ('LinkerRNN' not in cfg.MODEL.PRETRAIN_FILE) and training:
    ...
    if 'LinkerRNN' in cfg.MODEL.PRETRAIN_FILE and training:
