...
def list_sequences(root, set_ids):
    ...
    for s in set_ids:
        ...
        if root.startswith('s3://'):  # ceph
            sequences_cur_set = [(s, os.path.splitext(f)[0]) for f in client.list(anno_dir) if f.endswith('.txt')]
        else:
...
class TrackingNet(BaseVideoDataset):
    ...
    def _load_class_info(self):
        ...
        if class_map_path.startswith('s3://'):  # ceph
            raw = client.get(class_map_path).decode('utf-8')
            seq_to_class_map = {seq_class.split('\t')[0]: seq_class.rstrip().split('\t')[1] for seq_class in StringIO(raw)}
        else:
