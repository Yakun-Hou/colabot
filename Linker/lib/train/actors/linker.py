...
class LinkerActor(BaseActor):
    ...
    def __call__(self, data):
        ...
        if self.cfg.MODEL.SCORE_HEAD:
            loss, status = self.compute_score_loss(out_dict, data)
        else:
    ...
    def forward_pass(self, data):
        ...
        ps_img = data['ps_images'][0].view(-1, *data['ps_images'].shape[2:])  # (batch, 3, 320, 320)
        ...
        debug = False
        out_dict, debug_info = self.net(template=template_list,
                                        ps=ps_img,
                                        search=search_img,
        ...
        if debug:
            print(data['dataset'][0], data['search_visible'][0])
            gt_bbox = data['search_anno'][-1][0].cpu()
            x1, y1, w, h = gt_bbox
            gt_bbox = [x1 + w / 2, y1 + h / 2, w, h]
            ps_gt_bbox = data['ps_anno'][-1][0].cpu()
            x1, y1, w, h = ps_gt_bbox
            ps_gt_bbox = [x1 + w / 2, y1 + h / 2, w, h]
            linker_vis(debug_info, gt_bbox=gt_bbox, ps_gt_bbox=ps_gt_bbox)
        del debug_info
