...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
                 num_search_frames, num_ps_frames=1, num_template_frames=1, processing=no_processing, frame_sample_mode='causal',
        ...
        self.num_ps_frames = num_ps_frames
    ...
    def _sample_visible_ids(self, visible, num_ids=1, min_id=None, max_id=None,
        ...
        args:
            ...
            max_id - Maximum allowed frame number, sample in [min_id, max_id)
        ...
        # list = np.random.choice(valid_ids, num_ids, replace=True)
        try:
            list = np.random.choice(valid_ids, size=num_ids, replace=False, p=None).tolist()
        except:
            list = np.random.choice(valid_ids, size=num_ids, replace=True, p=None).tolist()            
        list.sort()
        return list
    ...
    def getitem(self):
        ...
        while not valid:
                ...
                if self.frame_sample_mode == 'causal':
                    ...
                    while search_frame_ids is None:
                        template_frame_ids = self._sample_visible_ids(visible, num_ids=1,
                                                                      ...
                                                                      max_id=len(visible) - 6)
                        # TODO: multiple ps frames
                        ps_frame_ids = self._sample_visible_ids(visible, num_ids=self.num_ps_frames,
                                                                min_id=template_frame_ids[0] + 1,
                                                                max_id=len(visible) - 1)
                        if ps_frame_ids is None:
                            continue
                        search_frame_ids = self._sample_visible_ids(visible, num_ids=self.num_search_frames,
                                                                    min_id=ps_frame_ids[0] + 1,
                                                                    max_id=ps_frame_ids[0] + self.max_gap + gap_increase)
                        gap_increase += 5
            ...
            if 1:
                template_frames, template_anno = dataset.get_frames(seq_id, template_frame_ids,
                                                                                    seq_info_dict)
                ps_frames, ps_anno = dataset.get_frames(seq_id, ps_frame_ids, seq_info_dict)
                search_frames, search_anno = dataset.get_frames(seq_id, search_frame_ids, seq_info_dict)

                data = TensorDict({'template_images': template_frames,
                                   'template_anno': template_anno['bbox'],
                                   'ps_images': ps_frames,
                                   'ps_anno': ps_anno['bbox'],
    ...
    def getitem_cls(self):
        ...
        while not valid:
            ...
            while search_frame_ids is None:
                template_frame_ids = self._sample_visible_ids(visible, num_ids=1,
                                                              min_id=self.num_template_frames - 1,
                                                              max_id=len(visible) - 2)
                ps_frame_ids = self._sample_visible_ids(visible, num_ids=1,
                                                        min_id=template_frame_ids[0] + 1,
                                                        max_id=len(visible) - 1)
                if ps_frame_ids is None:
                    continue
            ...
            try:
                if random.random() < self.pos_prob:
                    search_visible = True
                    pre_search_frames, pre_search_anno, _ = dataset.get_frames(seq_id, ps_frame_ids, seq_info_dict)
                    ...
                    if search_frame_ids is None:
                        ...
                        # TODO: ps 采样有问题
                        # ps_frame_ids = None
                        # for i in range(search_frame_ids[0]-1, -1, -1):
                        #     if visible[i]:
                        #         ps_frame_ids = [i]
                        #         break
                        # if ps_frame_ids is None:
                        ...
                        # ps_frame_ids = self._sample_visible_ids(visible, num_ids=1,
                        #                                         min_id=search_frame_ids[0] - 25,
                        #                                         max_id=search_frame_ids[0] + 25)
                        # if ps_frame_ids is None:
                        #     continue
                ...
                data = TensorDict({'template_images': template_frames,
                                   'template_anno': template_anno['bbox'],
                                   'ps_images': pre_search_frames,
                                   'ps_anno': pre_search_anno['bbox'],
                                   'search_images': search_frames,
                                   'search_anno': search_anno['bbox'],
                                   'dataset': dataset.get_name(),
...
class LinkerTSampler(TrackingSampler):
        ...
        while not valid:
            ...
            if is_video_dataset:
                ...
                template_frame_ids = self._sample_visible_ids(
                    visible, num_ids=1,
                    max_id=len(visible) - max_gap * self.num_search_frames)
                ...
                search_frame_ids = self._sample_visible_ids(
                    ...
                    max_id=base_frame_id[0] + max_gap * self.num_search_frames)
                if search_frame_ids is None:
                    continue
            else:
                # In case of image dataset, just repeat the image to generate synthetic video
    ...
    def getitem_cls(self):
        ...
        while not valid:
            ...
            template_frame_ids = self._sample_visible_ids(visible, num_ids=1,
                                                          max_id=len(visible) - self.max_gap * self.num_search_frames)
            base_id = template_frame_ids[0]
            search_frame_ids = []
            for i in range(self.num_search_frames - 1):
                search_frame_ids += self._sample_visible_ids(visible, num_ids=1, allow_invisible=True,
                                                             min_id=base_id + 1,
                                                             max_id=base_id + self.max_gap + 1)
                base_id = search_frame_ids[-1]
            ...
            try:
                if random.random() < self.pos_prob:
                    search_visible = True
                    frame_id = self._sample_visible_ids(visible, num_ids=1, min_id=search_frame_ids[-1] + 1)
                    frame, anno, meta_obj_test = dataset.get_frames(seq_id, frame_id, seq_info_dict)
                    search_frames += frame
                    for k, v in anno.items():
                        search_anno[k] += v
                else:
                    search_visible = False
                    frame_id = self._sample_visible_ids(visible, num_ids=1, min_id=search_frame_ids[-1] + 1, force_invisible=True)
                    if frame_id is None:
                        frame, anno, meta_obj_test = self.get_one_search()
                    else:
                        frame, anno, meta_obj_test = dataset.get_frames(seq_id, frame_id, seq_info_dict)
                        anno["bbox"] = [self.get_center_box(H, W)]
                    search_frames += frame
                    for k, v in anno.items():
                        search_anno[k] += v
                import ipdb; ipdb.set_trace()
...
class LinkerT2Sampler(TrackingSampler):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.max_gap = 4
    ...
    def getitem(self):
        ...
        while not valid:
            ...
            try:
                ...
                data = TensorDict({'template_images': template_frames,
                                   'template_anno': template_anno['bbox'],
                                   'template_pre_anno': template_anno['bbox'],
                                   'search_pre_anno': search_anno['bbox'][:-1],
                                   'search_images': search_frames[1:],
                                   'search_anno': search_anno['bbox'][1:],
                                   'dataset': dataset.get_name(),
                                   'test_class': meta_obj_test.get('object_class_name')})
                # make data augmentation
                data = self.processing(data)
