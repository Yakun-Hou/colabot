...
class Linker(BaseTracker):
    ...
    def track(self, image, info: dict = None):
        ...
        update_ps = max_score >= 0.7 and pred_score >= 0.7
        # print(max_score, '     ' , pred_score)
        # update_ps = max_score >= 0.6 and pred_score >= 0.7
        # update_ps = pred_score >= 0.7
        # update_ps = (self.interval == 20) and self.update
        # if update_ps and pred_score > self.max_pred_score:
        # if update_ps and max_score > self.max_pred_score:
        #     ps_patch_arr, _ = sample_target(image, self.state, self.params.search_factor,
        #                                     output_sz=self.params.search_size)
        #     self.online_max_template = self.preprocessor.process(ps_patch_arr)
        #     self.ps_idx_online = self.frame_id
        #     self.max_pred_score = max_score
        # if self.frame_id % self.update_interval == 0 and self.max_pred_score > 0:
        #     if self.online_size == 1:
        #         self.ps = self.online_max_template
        #         self.ps_idx = self.ps_idx_online
        #         self.max_pred_score = 0
        # import ipdb; ipdb.set_trace()
        # sum_score = max_score*1.0 + pred_score*1.0
        if self.debug and 0:
            # print('max_score:',max_score,' ','pred_score:', pred_score,' ', 'sum_score:', sum_score, 'max_pred_score:', self.max_pred_score)
            # cv2.putText(image, '#%03d' % self.frame_id, (10, 80),      
            # cv2.FONT_HERSHEY_SIMPLEX, 3, (255, 0, 0), 2, cv2.LINE_AA)
            cv2.putText(image, '#%03d' % pred_score, (10, 80),      
            cv2.FONT_HERSHEY_SIMPLEX, 3, (255, 0, 0), 2, cv2.LINE_AA)
            imgcat(image)
        if update_ps and pred_score > self.max_pred_score:
            # import ipdb; ipdb.set_trace()
        ...
        # if update_ps and self.frame_id%40==0:
        # # if update_ps:
        #     ps_patch_arr, _ = sample_target(image, self.state, self.params.search_factor,
        #                                     output_sz=self.params.search_size)
        #     self.ps_patch_arr = ps_patch_arr
