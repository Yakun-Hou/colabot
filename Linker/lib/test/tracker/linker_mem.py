...
from lib.models.ostrack import build_linker_mem
...
class LinkerMem(BaseTracker):
    def __init__(self, params, dataset_name):
        super().__init__(params)
        network = build_linker_mem(params.cfg, training=False)
        network.load_state_dict(torch.load(self.params.checkpoint, map_location='cpu')['net'], strict=True)
        # print(network.backbone.blocks[0].temporal_attn.q.weight)
        # print(network.backbone.blocks[1].temporal_attn.k.weight)
        # print(network.backbone.blocks[2].temporal_attn.v.weight)
        # print(network.backbone.blocks[3].temporal_attn.proj.weight)
        ...
        if self.debug:
            if not self.use_visdom:
                ...
                # self.add_hook()
                self._init_visdom(visdom_info={'server': 'http://cluster-proxy.sh.sensetime.com', 'port': 20539}, debug=1)
    ...
    def track(self, image, info: dict = None):
        ...
        with torch.no_grad():
            # import ipdb; ipdb.set_trace()
            out_dict = self.network.forward(self.template, self.ps, self.memory, search, ce_template_mask=self.box_mask_z)

        self.ps = out_dict['l_kv']
        ...
        #     self.ps = copy.deepcopy(out_dict['s_feat'])
