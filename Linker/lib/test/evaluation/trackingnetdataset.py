...
from lib.test.utils.ceph_utils import listdir  # ceph
...
class TrackingNetDataset(BaseDataset):
    ...
    def _construct_sequence(self, set, sequence_name):
        ...
        frame_list = [frame for frame in listdir(frames_path) if frame.endswith(".jpg")]
    ...
    def _list_sequences(self, root, set_ids):
        ...
        for s in set_ids:
            ...
            sequences_cur_set = [(s, os.path.splitext(f)[0]) for f in listdir(anno_dir) if f.endswith('.txt')]
