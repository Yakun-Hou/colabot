...
class GOT10KDataset(BaseDataset):
    ...
    def _construct_sequence(self, sequence_name):
        ...
        if 's3://' in self.base_path:  # ceph
            frame_list = [frame for frame in client.list(frames_path) if frame.endswith(".jpg")]
        else:
    ...
    def _get_sequence_list(self, split):
        if 's3://' in self.base_path:  # ceph
            url = os.path.join(self.base_path, 'list.txt')
            raw = client.get(url).decode('utf-8')
            sequence_list = raw.splitlines()
        else:
